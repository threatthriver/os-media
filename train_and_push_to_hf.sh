...
# and automatically push checkpoints to Hugging Face Hub
...
# Run the training with optimized parameters and HF Hub integration
python tpu_train.py \
  --model_size 600b \
  ...
  --log_memory_usage \
  --push_to_hub \
  --hub_model_id threatthriver/llm-600b-128k \
  --hub_token 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 \
  --save_steps 1000 \
